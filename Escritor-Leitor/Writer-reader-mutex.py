...
queue = threading.Semaphore(1)  # Controla a ordem de acesso (fairness)

# Função para leitores
def reader(id):
    global read_count
    while True:
        # Controle de acesso justo
        queue.acquire()  # Entram na fila de acesso
        mutex.acquire()
        read_count += 1
        if read_count == 1:
            write.acquire()  # Bloqueia escritores se for o primeiro leitor
        mutex.release()
        queue.release()  # Sai da fila de acesso
